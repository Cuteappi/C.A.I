...
import { GetInputAxis, CheckKeyType } from "./Utility";
import { compose, ModifierFactories, ModifierFunction, Mods } from "./Modifiers";
import { DownTrigger } from "./Triggers/DownTrigger";
...
export type TriggerType = "DownTrigger" | "PressTrigger";
...
export class InputMapping {
	...
	constructor(
		...
		this.Trigger = this.InitTrigger(trigger);
		this.Axis = axis;
	}
	...
	private ProcessInput(): Vector3 {
		...
		if (!KeyData) { warn("KeyData not found"); return Vector3.zero; }
	...
	public InitTrigger(trigger: TriggerType): BaseTrigger {
		switch (trigger) {
			case "DownTrigger":
